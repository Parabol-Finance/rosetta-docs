...
```jsx
time_elapsed = current_timestamp - last_update_timestamp
linear_rate = variable_borrow_rate * time_elapsed / seconds_per_year

# Liquidity index update (ray math, 10^27 precision)
liquidity_index = liquidity_index_previous * (1 + linear_rate * utilization)
```
...
```jsx
supply_apy = (1 + variable_borrow_rate * utilization / seconds_per_year) ^ seconds_per_year - 1
```
...
All metrics are recomputed every block. On HyperEVM with ~1 second block times, this means near real-time accuracy. On Base, block times are ~2 seconds.
